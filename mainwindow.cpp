    ...
    , ui(new Ui::MainWindow)
{
    ...
    QPushButton *emptyBt = new QPushButton(" ", this);
    emptyBt->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
    emptyBt->setStyleSheet("QPushButton:{border-radius: 10px; font: bold 14px; background-color: #8B8A8A}"
                           "QPushButton:hover{background: #A9A4A4;}"
                           "QPushButton:pressed{background: #A9A4A4; border-width: 1px; border-style: solid; border-radius: 10px; border-color: CECBCB}"
                           );
    ...
    main_box->addWidget(leftBkt, 1, 0);
    main_box->addWidget(rightBkt, 1, 1);
    main_box->addWidget(emptyBt, 1, 2);
